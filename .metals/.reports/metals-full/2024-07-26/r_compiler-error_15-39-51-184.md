file://<WORKSPACE>/src/main/scala/progfun/utils/ConfigReader.scala
### java.lang.AssertionError: assertion failed: position error, parent span does not contain child span
parent      = new Exception(null: <notype>) # -1,
parent span = <955..990>,
child       = null # -1,
child span  = [969..995..995]
...
action parameters:
uri: file://<WORKSPACE>/src/main/scala/progfun/utils/ConfigReader.scala
text:
```scala
...
object ConfigReader {
  def readConfig(configPath: String): Try[Config] = {
    val source = Source.fromFile(configPath)(scala.io.Codec.UTF8)
    val lines =
      try source.mkString
  ...
  private def parseConfig(json: String): Try[Config] = {
    val pattern =
      """\{\s*"input-file":\s*"(.+?)",\s*"output-json-file":\s*"(.+?)",\s*"output-csv-file":\s*"(.+?)",\s*"output-yaml-file":\s*"(.+?)"\s*\}""".r

    json match {
      case pattern(inputFile, outputJsonFile, outputCsvFile, outputYamlFile) =>
        Success(
          Config(inputFile, outputJsonFile, outputCsvFile, outputYamlFile)
        )
        //  [wartremover:Nothing] Inferred type containing Nothing: Nothing
      case _ => Failure(new Exception("Failed to parse JSON
    }
  }
}
